...
class SpecData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == "train":
            ...
            # resamp_mel_specgram = self.re_speclist[item]
            noisy_mel_specgram = self.noise_speclist[item]
            ir_mel_specgram = self.irlist[item]

            # 随机按位置切
            cut_len = resample_rate * 5 // HOP_LENGTH
            ...
            image = mel_out
            mel_out = np.concatenate((image, image, image), axis=0)

            image = mel_corruput_out
            mel_corruput_out = np.concatenate((image, image, image), axis=0)
        ...
        if self.mode == "test":
            ...
            image = mel_specgram
            mel_specgram = np.concatenate((image, image, image), axis=0)
